...
class LoadSaveMenu:
    def __init__(self, screen):
        ...
        self.saves = self.save_manager.get_all_saves()[:6]  # Ограничиваем до 6 сохранений
        
        # Выбранное сохранение
        self.selected_save_index = -1
        
        # Параметры скроллинга
        self.scroll_offset = 0
        self.max_visible_saves = 4  # Показываем 4 сохранения одновременно
        ...
        # Кнопки Start и Delete (показываются только при выборе сохранения)
        button_width = 140
        button_height = 50
        button_spacing = 20
    ...
    def create_save_buttons(self):
        """Создает кнопки для каждого сохранения"""
        self.save_buttons = []
        
        for i, save in enumerate(self.saves):
            # Цвет кнопки зависит от того, выбрана ли она
            ...
            button = Button(
                SCREEN_WIDTH // 2 - 400, 150 + i * 120,  # Увеличиваем ширину и высоту
                800, 100,  # Увеличиваем размеры
                save["save_name"], color, hover_color
            )
            self.save_buttons.append(button)
    
    def refresh_saves(self):
        """Обновляет список сохранений и кнопки"""
        self.saves = self.save_manager.get_all_saves()[:6]
        self.selected_save_index = -1
        self.create_save_buttons()
    
    def draw_save_info(self, save, button_rect):
        """Отрисовывает информацию о сохранении"""
        # Название сохранения (уже отрисовано в кнопке)
    ...
    def handle_scroll(self, event):
        """Обрабатывает скролл для большого количества сохранений"""
    ...
    def run(self):
        ...
        while running:
            ...
            # Обработка кнопок сохранений
            for i, button in enumerate(self.save_buttons):
                if i >= len(self.saves):
                    break
                    
                # Корректируем позицию кнопки с учетом скролла
                ...
                if button.is_clicked(mouse_pos, mouse_click):
                    ...
                    self.create_save_buttons()  # Пересоздаем кнопки с обновленными цветами
            
            # Обработка кнопок Start и Delete (только если выбрано сохранение)
            if self.selected_save_index >= 0:
                ...
                if self.delete_button.is_clicked(mouse_pos, mouse_click):
                    ...
                    if self.save_manager.delete_save_by_filename(filename):
                        self.refresh_saves()  # Обновляем список
            ...
            if not self.saves:
                ...
                # Отрисовываем кнопки сохранений
                for i, button in enumerate(self.save_buttons):
                    if i >= len(self.saves):
                        break
                ...
                # Показываем индикатор скролла, если нужно
            ...
            # Кнопки Start и Delete (только если выбрано сохранение)
            if self.selected_save_index >= 0:
                self.start_button.draw(self.screen)
                self.delete_button.draw(self.screen)
                
                # Показываем сообщение о том, что сохранение выбрано
