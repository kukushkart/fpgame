...
class ManageSavesPopup:
    def __init__(self, screen, save_manager):
        ...
        # Параметры для скроллинга
        self.scroll_offset = 0
        self.max_visible_saves = 3  # Максимум видимых сохранений на экране
        self.save_area_height = 360  # Высота области для сохранений
        self.save_area_top = 150    # Верхняя позиция области сохранений
        ...
        # Делаем кнопки более центрированными
        button_width = 140
        button_height = 50
        button_spacing = 30
    ...
    def run(self):
        ...
        while running:
            ...
            # Обработка кликов по сохранениям с учетом скроллинга
            for i, button in enumerate(self.save_buttons):
                # Обновляем позицию кнопки с учетом скроллинга
                button.rect.y = self.save_area_top + i * 120 - self.scroll_offset
                
                # Проверяем, видна ли кнопка на экране
            ...
            if self.selected_save_index != -1:
                ...
                if self.delete_button.is_clicked(mouse_pos, mouse_click):
                    ...
                    self.selected_save_index = -1
                    # Корректируем скроллинг если нужно
                    max_scroll = max(0, (len(self.saves) - self.max_visible_saves) * 120)
    ...
    def draw(self):
        ...
        # Сообщение о лимите - перенесли выше, чтобы не накладывалось
        ...
        # Рисуем полосу прокрутки если есть много сохранений
        ...
        if len(self.saves) < 6:
            ...
            # Показываем сообщение что нужно удалить сохранение - перенесли выше кнопок
